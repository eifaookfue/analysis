package jp.co.nri.nefs.tool.analytics.collect.client
...
class ClientLogCollector(isZip: Boolean, searchPath: Path, outputPath: Path) extends LazyLogging{
...
object ClientLogCollector {
  type OptionMap = Map[Symbol, String]
  val usage = """
        Usage: jp.co.nri.nefs.tool.analytics.collect.client.ClientLogCollector [--nozip] [--searchdir dir] [--outputdir dir]
        """
  def main(args: Array[String]): Unit = {
    ...
    val clientLogCollector = new ClientLogCollector(isZip, searchdir, outputdir)
    clientLogCollector.collect()
  }
