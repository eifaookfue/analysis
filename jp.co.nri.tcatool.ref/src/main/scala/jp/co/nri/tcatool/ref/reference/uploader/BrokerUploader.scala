...
import jp.co.nri.tcatool.ref.reference.model.{Broker, BrokerComponent}
import play.api.db.slick.{DatabaseConfigProvider, HasDatabaseConfigProvider}
import slick.jdbc.JdbcProfile

import scala.util.Try

class BrokerUploader @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  extends BrokerComponent with SimpleUploader[Broker] with DBUtil[Broker]
  with HasDatabaseConfigProvider[JdbcProfile] with LazyLogging {

  import profile.api._

  val query = TableQuery[Brokers]

  override def convert(row: List[String]): Try[Broker] = {
    for {
      brokerId <- Read[Int].reads(row.head)
      brokerName <- Read[String].reads(row(1))
      abbreviation <- Read[String].reads(row(2))
      updateTime = new Timestamp(System.currentTimeMillis())
      broker = Broker(brokerId, brokerName, abbreviation, updateTime)
    } yield broker
  }
}

object BrokerUploader extends LazyLogging {
  final val CONFIG_BASE = "BrokerUploader"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val brokerUploader = ServiceInjector.getComponent(classOf[BrokerUploader])
    brokerUploader.upload(input, splitter, charSet, uploadPerCount, startNumber, maxNumberOfLine)
  }
