...
class WindowSpec extends FlatSpec with PrivateMethodTester with TestingEnvironment with LogCollection {
  ...
  "NewSplitDialog" should "be bound to SmartSplitDialog when it raised from SmartSplit" in {
    ...
    val result = logAnalyzer invokePrivate findRelatedWindow(windowBuffer, lineInfo2.underlyingClass)
  ...
  "SelectMultiDialog" should "NOT be bound to anything" in {
    ...
    val result = logAnalyzer invokePrivate findRelatedWindow(windowBuffer, lineInfo1.underlyingClass)
    assert(result === None)
  }
  ...
  "NewSplitDialog" should "have end property in encounter of close message" in {
    ...
    logAnalyzer invokePrivate updateWithEnd(lineInfo2.windowName, windowBuffer, "", lineNo2,f)
  ...
  it should "have end property even if which raised in nesting" in {
    ...
    logAnalyzer invokePrivate updateWithEnd(lineInfo3.windowName, windowBuffer, "", lineNo3,f3)
    ...
    logAnalyzer invokePrivate updateWithEnd(lineInfo4.windowName, windowBuffer, "", lineNo4,f4)
